#!/usr/bin/env python3
"""
Video Transcription Pipeline V06 for Educational Research
...
NEW IN V06:
- Updated to support gemini-2.0-flash-exp and other new Gemini models
- All V04 features maintained (Hybrid VAD, denoising, etc.)
...
class TranscriptionConfigV04:
    """Enhanced configuration for V06 with VAD preprocessing"""
    # Core settings
...
class VADEnhancedConsensusAnalyzer:
    ...
    def _flag_with_vad_confidence(self, aligned_data: List[Dict]) -> List[Dict]:
        ...
        for alignment in aligned_data:
            ...
            # Determine flag with VAD consideration
            if enhanced_speaker_conf >= 0.7 and enhanced_content_conf >= 0.75:
                ...
                flag = "⚠️"
                review_needed = True
                flag_reason = f"👤{enhanced_speaker_conf*100:.0f}💬{enhanced_content_conf*100:.0f}📊{vad_confidence:.2f}"
            else:
                flag = "🚨"
                review_needed = True
                flag_reason = f"👤{enhanced_speaker_conf*100:.0f}💬{enhanced_content_conf*100:.0f}📊{vad_confidence:.2f}"
...
class VADEnhancedTranscriber:
    ...
    def __init__(self, api_key: str, config: TranscriptionConfigV04):
        ...
        # Load enhanced prompts with correct path
    ...
    def _ensure_vad_prompts(self):
        ...
        # Only add default VAD prompt if using the default enhanced_vad prompt
...
"""
    ...
    def _transcribe_with_vad_consensus(self, uploaded_file, chunk_info: Dict, 
        ...
        for run_num in range(1, self.config.consensus_runs + 1):
            ...
            if is_valid:
                ...
                # Display the individual run
                print(f"\n  📋 TRANSCRIPT RUN {len(transcript_runs)}:")
                print(f"  {'-' * 60}")
                # Show first 15 lines of the transcript
            ...
            try:
                ...
                response = self.model.generate_content(
                    ...
                    generation_config={
                        ...
                        "max_output_tokens": 8192,  # Increased from 4096 for longer videos
                    }
                )
    ...
    def _create_vad_context(self, chunk_info: Dict) -> str:
        ...
        # If VAD preprocessing is disabled, return minimal context
...
class VideoTranscriptionPipelineV04:
    ...
    def __init__(self, api_key: str, config: TranscriptionConfigV04, skip_confirmation: bool = False):
        self.config = config
    ...
    def process_video(self, video_path: str, output_dir: str = None) -> Dict:
        ...
        if not self.skip_confirmation:
            ...
            print("\n🚀 Starting V04 VAD-enhanced transcription (confirmation skipped)...")
        ...
        try:
            ...
            for chunk_info in chunk_info_list:
                ...
                chunk_file = output_dir / f"chunk_{chunk_number:02d}_consensus_transcript.txt"
                with open(chunk_file, 'w', encoding='utf-8') as f:
                    f.write(transcript)
                
                # Also save with the old naming for compatibility
                chunk_file_legacy = output_dir / f"chunk_{chunk_number:02d}_v04_transcript.txt"
                with open(chunk_file_legacy, 'w', encoding='utf-8') as f:
                    f.write(transcript)
...
def main():
    """Main entry point for V06 pipeline"""

    parser = argparse.ArgumentParser(
        description="V06 Video Transcription Pipeline with Hybrid VAD Enhancement & New Gemini Models",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
V06 ENHANCEMENTS:
  • Support for gemini-2.0-flash-exp and other new Gemini models
...
EXAMPLES:
  Basic V06 processing with new model:
    python video_transcription_pipeline_v06.py video.mp4

  Use specific model:
    python video_transcription_pipeline_v06.py video.mp4 --model gemini-2.0-flash-exp

  With multiple consensus runs:
    python video_transcription_pipeline_v06.py video.mp4 --consensus-runs 3

  Skip confirmation prompt for automation:
    python video_transcription_pipeline_v06.py video.mp4 --no-confirm

  Disable VAD preprocessing (fallback to V03 mode):
    python video_transcription_pipeline_v06.py video.mp4 --no-vad
        """
    )
    ...
    parser.add_argument("-m", "--model", default="gemini-2.0-flash-exp", help="Gemini model (default: gemini-2.0-flash-exp)")
    ...
    try:
        if args.estimate_only:
            ...
            processor = VideoTranscriptionPipelineV04(api_key, config, skip_confirmation=args.no_confirm)
